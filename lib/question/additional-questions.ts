// Additional feasible questions for MatchMind dApp
// Based on deeper analysis of SportMonks API capabilities

import { FEASIBLE_QUESTIONS } from './question-analysis'
...
export const ADDITIONAL_FEASIBLE_QUESTIONS: AdditionalQuestion[] = [
...
export const HIGH_ENGAGEMENT_QUESTIONS = ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.engagement === 'high')
export const MEDIUM_ENGAGEMENT_QUESTIONS = ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.engagement === 'medium')

// Filter by difficulty
export const EASY_QUESTIONS = ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.difficulty === 'easy')
export const MEDIUM_QUESTIONS = ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.difficulty === 'medium')
export const HARD_QUESTIONS = ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.difficulty === 'hard')

// Get questions by category
export function getQuestionsByCategory(category: string): AdditionalQuestion[] {
  return ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => q.category === category)
}

// Get all additional questions
export function getAllAdditionalQuestions(): AdditionalQuestion[] {
  return ADDITIONAL_FEASIBLE_QUESTIONS
}

// Get recommended questions for the game
export function getRecommendedQuestions(): AdditionalQuestion[] {
  return ADDITIONAL_FEASIBLE_QUESTIONS.filter(q => 
    q.engagement === 'high' || 
    (q.engagement === 'medium' && q.difficulty !== 'hard')
  )
} 